...
## ⚙️ Описание скриптов
1. generateEmails.js - генерирует почты. Проблема в том, что iCloud разрешает создавать максимум 5 почт, после чего лочит эту функцию на 2 часа. Скрипт создает указанное количество почт, ждем 2.2 часа, продолжает. Вам нужно изменить следующие переменные:
